...
<head>
	...
	<link rel="stylesheet" href="<?php echo base_url(); ?>styles/actionlog_graph.css?<?php echo date('YmdHHmmss'); ?>" type="text/css" />
			...
			<div id="result_area" class="result_area">
				<div id="result_data" class="result_data" >
					<div class="result_data_title">
						<span v-html="ymdDisp + ' 月間グラフ'"></span>
					</div>
		...
		<script>
			...
			var labelItems = [
				...
				foreach ($actionLogRecords as $item) {
					echo convDateTime($item["ymd"])->format('j') . ",";
				}
				?>
			];
			var cntItems = [
			...
			var sumTimeItems = [
				...
				$maxSumTime = 0;
				foreach ($actionLogRecords as $item) {
					if ($maxSumTime < $item["sumtime"]) {
						$maxSumTime = $item["sumtime"];
					}
				}
				$unit = 0;
				$unitName = 0;
			...
			if (window.innerWidth > window.innerHeight) {
				// 横長の場合
				$("#graph_body").attr('style', "position: relative;padding:0px;margin:0px; height:500px;width:<?= count($actionLogRecords) * 60 ?>px");
			} else {
				// 縦長の場合
				$("#graph_body").attr('style', "position: relative;padding:0px;margin:0px; height:700px;width:<?= count($actionLogRecords) * 60 ?>px");
			}
					...
					tooltips: {
						...
						callbacks: {
							title: function(tooltipItem, data) {
								// ツールチップ表示時に対象日付をVueにセット
								day_action_log_list.selectDay = tooltipItem[0].xLabel;

								return tooltipItem[0].xLabel + " 日";
							},
							label: function(tooltipItem, data) {
								if (tooltipItem.datasetIndex == 0) {
									return ["合計回数:" + tooltipItem.yLabel + " 回"];
								} else {
