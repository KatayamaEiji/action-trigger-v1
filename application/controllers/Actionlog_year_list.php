 ...
 * アクションログリスト
 */
class Actionlog_year_list extends MY_Controller {

	/**************************************************
	 * constructor / destructor
	 **************************************************/
	function __construct(){
		parent::__construct(self::ACTION_ACTIONLOG_YEAR_LIST);
	}
	...
	public function index($reDispId){
		...
		$this->actionLogYearListLoadView($userId,$actionlogfilterParams);
	}
	 ...
	 **************************************************/
	/**
	 * actionLogYearListLoadView
	 */
	private function actionLogYearListLoadView($userId,$actionlogfilterParams){
		$this->load->model(array("actionlog_year_list_manager"));

		// ログタイプ設定
		$actionlogfilterParams->set_log_type(Actionlogfilter_params_entity::LOG_TYPE_LIST);

		// アクションログフィルターのパラメータ設定
		$actionlogfilterParams->set_disp_id($this->m_items['dispId'],$this->m_items['reDispId']);

		// アクションログデータを取得
		$actionlogRecords = $this->actionlog_year_list_manager->getActionYearLogList($userId,$actionlogfilterParams);
		...
		$this->loadView(self::ACTION_ACTIONLOG_YEAR_LIST);
	}
}