 ...
 * アクションロググラフ
 */
class Actionlog_graph extends MY_Controller {

	/**************************************************
	 * constructor / destructor
	 **************************************************/
	function __construct(){
		parent::__construct(self::ACTION_ACTIONLOG_GRAPH);
	...
	private function actionLogGraphLoadView($userId,$actionlogfilterParams){
		$this->load->model(array("actionloggraph_manager","actionlog_manager"));

		// ログタイプ設定
		$actionlogfilterParams->set_log_type(Actionlogfilter_params_entity::LOG_TYPE_GRAPH);

		// アクションログデータを取得
		$actionLogRecords = $this->actionloggraph_manager->getActionLogGraph($userId,$actionlogfilterParams);
		...
		$this->loadView(self::ACTION_ACTIONLOG_GRAPH);
	}
}